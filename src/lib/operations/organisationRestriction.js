 ...
 */
function _create(body, id, options, callback) {
  ...
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'api/organisations/{id}/restrictions';
  ...
  try {
    if (body !== null && body !== undefined) {
      let requestModelMapper = new client.models['Restriction']().mapper();
    ...
    if (statusCode === 200) {
      ...
      try {
        ...
        if (parsedResponse !== null && parsedResponse !== undefined) {
          let resultMapper = new client.models['Restriction']().mapper();
 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _deleteMethod(id, targetOrganisationId, options, callback) {
   /* jshint validthis: true */
  let client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
  ...
  try {
    ...
    if (targetOrganisationId === null || targetOrganisationId === undefined || typeof targetOrganisationId.valueOf() !== 'string') {
      throw new Error('targetOrganisationId cannot be null or undefined and it must be of type string.');
    }
    if (xAmphoradataVersion !== null && xAmphoradataVersion !== undefined && typeof xAmphoradataVersion.valueOf() !== 'string') {
      throw new Error('xAmphoradataVersion must be of type string.');
    }
  ...
  // Construct URL
  let baseUrl = this.client.baseUri;
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'api/organisations/{id}/restrictions/{targetOrganisationId}';
  requestUrl = requestUrl.replace('{id}', encodeURIComponent(id));
  requestUrl = requestUrl.replace('{targetOrganisationId}', encodeURIComponent(targetOrganisationId));
  ...
  return client.pipeline(httpRequest, (err, response, responseBody) => {
    ...
    if (statusCode === 200) {
      ...
      try {
        ...
        result = JSON.parse(responseBody);
        if (parsedResponse !== null && parsedResponse !== undefined) {
          let resultMapper = new client.models['GenericResponse']().mapper();
          result = client.deserialize(resultMapper, parsedResponse, 'result');
        }
      } catch (error) {
        let deserializationError = new Error(`Error ${error} occurred in deserializing the responseBody - ${responseBody}`);
...
/** Class representing a OrganisationRestriction. */
class OrganisationRestriction {
  /**
   * Create a OrganisationRestriction.
   ...
   */
  createWithHttpOperationResponse(body, id, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._create(body, id, options, (err, result, request, response) => {
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  create(body, id, options, optionalCallback) {
    let client = this.client;
    let self = this;
    ...
    if (!optionalCallback) {
      ...
      return self._create(body, id, options, optionalCallback);
    }
  }
   ...
   * @reject {Error} - The error object.
   */
  deleteMethodWithHttpOperationResponse(id, targetOrganisationId, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._deleteMethod(id, targetOrganisationId, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  deleteMethod(id, targetOrganisationId, options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._deleteMethod(id, targetOrganisationId, options, optionalCallback);
    }
  }

}

module.exports = OrganisationRestriction;
